...
func (data *TestData) deployFlowVisibilityClickHouse() (*PodIPs, error) {
	...
	if err := wait.Poll(2*time.Second, 10*time.Second, func() (bool, error) {
		rc, stdout, stderr, err := provider.RunCommandOnNode(controlPlaneNodeName(), fmt.Sprintf("kubectl apply -f %s", flowVisibilityYML))
		if err != nil || rc != 0 {
	...
	if err != nil {
		_, stdout, stderr, podErr := provider.RunCommandOnNode(controlPlaneNodeName(), fmt.Sprintf("kubectl get po %s -n %s -o yaml", podName, kubeNamespace))
	...
	if err != nil {
		_, stdout, stderr, podErr := provider.RunCommandOnNode(controlPlaneNodeName(),
...
func (data *TestData) deleteClickHouseOperator() error {
	rc, _, _, err := provider.RunCommandOnNode(controlPlaneNodeName(), fmt.Sprintf("kubectl delete -f %s -n kube-system", clickHouseOperatorYMLUrl))
	if err != nil || rc != 0 {
...
func (data *TestData) deployFlowAggregator(ipfixCollector, clickHouse string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to get service %s: %v", flowAggregatorDeployment, err)
	}
	if err = data.mutateFlowAggregatorConfigMap(ipfixCollector, clickHouse, svc.Spec.ClusterIP); err != nil {
		return err
	}
	...
	if err != nil {
		_, stdout, stderr, podErr := provider.RunCommandOnNode(controlPlaneNodeName(), fmt.Sprintf("kubectl get po %s -n %s -o yaml", podName, flowAggregatorNamespace))
		return fmt.Errorf("error when waiting for flow-aggregator Ready: %v; stdout %s, stderr: %s, %v", err, stdout, stderr, podErr)
	}
	return nil
}

func (data *TestData) mutateFlowAggregatorConfigMap(ipfixCollector, clickHouse, faClusterIP string) error {
